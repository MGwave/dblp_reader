...
do
	...
	do
		...
		do
			output_file="outputSmallD/"$experiment_type"_"$dataset"_"$penalty_type"_"$tfidf_type"_5.txt"
		 	./topKQueryTest --"$experiment_type" "$dataset" "$dataset"_pos_pairs.txt $target_file 5 $penalty_type $tfidf_type > $output_file
		done
	done
	./topKQueryTest --"$experiment_type" "$dataset" "$dataset"_pos_pairs.txt $target_file 5 1 SP > "outputSmallD/"$experiment_type"_"$dataset"_SP_5.txt"
done
